...
void log_test(const std::string& name, std::function<void()> test_func) {
    ...
    try {
        ...
        std::cout << ">>> РЕЗУЛЬТАТ: [ОШИБКА] Исключение: " << e.what() << "\n";
    }
    catch (...) {
        std::cout << ">>> РЕЗУЛЬТАТ: [ОШИБКА] Неизвестная ошибка\n";
    }
}
...
void test1_basic_execution() {
    ...
    if (actual != -1) {
        throw std::runtime_error("Ожидалось -1, получено " + std::to_string(actual));
    }
}
...
void test3_default_values() {
    ...
    int actual = std::any_cast<int>(result);
    if (actual != 17) {
        throw std::runtime_error("Ожидалось 17, получено " + std::to_string(actual));
    }
}
...
void test4_command_not_found() {
    ...
    if (!exception_caught) {
        throw std::runtime_error("Ожидалось исключение runtime_error при отсутствии команды.");
    }
}
...
void test5_missing_required_argument() {
    ...
    if (!exception_caught) {
        throw std::runtime_error("Ожидалось исключение invalid_argument при отсутствии аргумента.");
    }
}
...
void test6_return_type_mismatch() {
    ...
    if (!exception_caught) {
        throw std::runtime_error("Ожидалось исключение bad_any_cast.");
    }
}
